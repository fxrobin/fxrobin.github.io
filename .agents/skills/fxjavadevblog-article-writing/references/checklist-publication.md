...
- [ ] Aucun tic IA (voir `references/anti-ia-tics.md`, y compris la section gimmicks)

## Typographie
- [ ] Aucun tiret long `—` ni tiret moyen `–`
- [ ] Aucun `…` Unicode - utiliser `...`
- [ ] Aucun guillemet courbe `"` `"` ni apostrophe typographique `'`
- [ ] Aucun `~`, `~+`, `+/-`, `±` en prose - approximation en toutes lettres (`~` toléré dans les chemins Unix et le code)
