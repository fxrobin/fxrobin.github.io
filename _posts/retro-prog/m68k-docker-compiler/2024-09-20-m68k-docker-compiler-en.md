...
> **Note**: Notice the use of `-u $(id -u):$(id -g)` to run the Docker container with the same permissions as the current user. This ensures that the files generated by the container retain the correct permissions.
...
Finally, I recommend creating a bash alias for this command to simplify its usage. For example, add the following line to your `~/.bashrc` file:

```bash
alias m68k='docker run -it -u $(id -u):$(id -g) -v $(pwd):/app fxrobin/m68k-compiler'
```

This alias allows you to run a simple `m68k make` command to compile the C program for the Atari ST:
