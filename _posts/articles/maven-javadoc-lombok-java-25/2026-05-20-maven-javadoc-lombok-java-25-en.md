...
The solution I built goes through **a delombok phase** (generating source code rather than bytecode), a precise Maven configuration for Java 25, a dedicated profile to avoid breaking IDEs, **Groovy** scripts to inject missing Javadoc into Lombok-generated classes, and as a bonus, two *skills* so you do not have to walk this path yourself.
...
The idea came naturally: **use Groovy to post-process the delomboked files** and inject the missing Javadoc on classes and methods generated by Lombok. In fact, I had already used Groovy successfully to [create a Thomson TO8 bootsector](/6809-thomson-to8-bootsector), which proves the versatility of this language.
