...
The problem is fundamental: Lombok is an annotation processor that modifies the AST (Abstract Syntax Tree) of the Java compiler during compilation. The final bytecode does contain the generated methods, but the source code remains unchanged. And `javadoc` reads source code, not bytecode. Your `@Builder`, `@UtilityClass`, and other annotations vanish into that gap - and with them, all the carefully written documentation on your fields.
...
> **A small digression that only reflects my own opinion**: I genuinely cannot understand developers, tech leads, and architects who resist Lombok with arguments like "we do not know what it does" or "it's for lazy people." Among Spring Boot developers, who really knows what `@Transactional`, `@GetMapping`, `@Component`, or a `JpaRepository` interface actually does under the hood? These Spring annotations do not generate bytecode at compile time: they work through **dynamic proxies at runtime** (Spring AOP, CGLIB) or are read reflectively by the framework. The mechanism is fundamentally different from Lombok, which is an **annotation processor** that modifies the compiler's AST to generate real code in the compiled class. Two different worlds, but in both cases, magic happens under the hood. Why would Lombok be wrong while the rest is acceptable? I have heard here and there that Lombok causes problems during refactoring. I have done plenty of refactorings, and Lombok was never the main problem - far from it. End of digression, you are free to disagree.

## What is the actual problem?

Lombok is an annotation processor that modifies the compiler's AST: **that is precisely where Javadoc generation breaks down**.

Javadoc is generated from source code, not from the bytecode produced by the compiler. The source code does not contain the methods generated by Lombok, so **Lombok annotations are simply not taken into account** by the javadoc tool.
...
> Why this approach? Because Lombok modifies the AST at compile time, not the source code. Delombok generates source, but without Javadoc. **Groovy can modify that source.** It is the missing piece of the puzzle.
